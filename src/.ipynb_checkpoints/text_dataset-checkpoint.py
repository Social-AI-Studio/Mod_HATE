...
class TEXT_Data():
    ...
    def __init__(self,opt,tokenizer,mode='train'):
        ...
        self.opt=opt
        self.mode=mode
        self.instruct="Please decide whether the sentence below is a hate speech."
    ...
    def organize_data(self,save_path):
        data_raw=load_json(os.path.join(self.opt.PATH,'domain_splits',
                                        'hate_all_'+self.mode+'.json'))
        captions=load_pkl(os.path.join('../BLIP-2/results/harm-generic.pkl'))
    ...
    def get_dataset(self):
        ...
        if self.mode=='test':
            data["train"]=data["train"].select([i for i in range(1000)])
